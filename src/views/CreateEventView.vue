  ...
  <div>
    ...
    <div class="max-w-3xl mx-auto px-4 py-8">
      <form @submit.prevent="handleSubmit" class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <!-- Status -->
        <div class="space-y-2">
          <label class="block text-sm font-medium text-slate-700">Statut souhaité <span class="text-red-500">*</span></label>
          <div class="grid grid-cols-2 gap-3">
            <button
              type="button"
              @click="form.intendedStatus = 'poll'"
              class="py-3 rounded-xl text-sm font-semibold border transition-all"
              :class="form.intendedStatus === 'poll'
            ...
            <button
              type="button"
              @click="form.intendedStatus = 'confirmed'"
              class="py-3 rounded-xl text-sm font-semibold border transition-all"
              :class="form.intendedStatus === 'confirmed'
                ? 'bg-emerald-50 border-emerald-300 text-emerald-700'
                : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'
              "
            >
          ...
          <div class="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            ...
            L'événement sera soumis à l'approbation d'un administrateur avant publication.
          </div>
        </div>
...
async function handleSubmit() {
  ...
  const eventData = {
    ...
    status: 'pending',
    intendedStatus: form.intendedStatus,
